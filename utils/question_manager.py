...
def calculate_score():
    ...
    # --- MODIFICACIÓN: Obtener nombre y email, y crear prefijo de log ---
    # Obtiene el nombre y el email del usuario del session state, o 'N/A' si no están disponibles.
    user_name = st.session_state.user_data.get('nombre', 'N/A')
    user_email = st.session_state.user_data.get('email', 'N/A')
    # Crea el prefijo que se añadirá a todas las líneas de log, incluyendo ambos datos.
    log_prefix = f"User: {user_name} ({user_email}) | "
    # --- FIN MODIFICACIÓN ---
    ...
    for idx, question in enumerate(questions):
        ...
        # --- MODIFICADO: Prepend prefix to original print ---
        # Añade el prefijo a la línea de depuración original.
        print(f"{log_prefix}Pregunta {}: Respuesta del usuario: {}, Respuesta correcta: {question['respuesta_correcta']}")  # DEBUG
        ...
        if user_answer is not None and user_answer in question["respuesta_correcta"]:
            ...
            # Añade el prefijo a la línea de depuración original.
            print(f"{log_prefix}Añadida respuesta incorrecta a la lista: {}")  # DEBUG

    # --- MODIFIED: Prepend prefix to original prints ---
    # Añade el prefijo a las líneas de depuración originales.
    print(f"{log_prefix}Total de respuestas correctas: {}")  # DEBUG
    ...
    # Nueva línea de depuración para mostrar el score final calculado, con el prefijo.
