...
def to_latin1(s: str) -> str:
    """Reemplaza caracteres fuera de rango Latin-1 por '?'."""
...
class CustomPDF(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(logo_path):
            self.image(logo_path, x=10, y=8, w=50)  # Ajusta x, y, w
        self.set_font("Arial", 'B', 15)
        self.cell(80)
        self.ln(20)  # Podrías necesitar ajustar esto

    def footer(self):
        """Pie de página con número de página y fecha/hora."""
        self.set_y(-15)
        self.set_font("Arial", 'I', 8)
        page_text = to_latin1(f"Page {self.page_no()}/{''}")
        self.cell(0, 5, page_text, align='C')
        self.set_y(-10)
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.cell(0, 5, to_latin1(f"Generated on: {timestamp}"), align='C')

# ... (resto de las funciones _draw_classification_row y get_feedback) ...
def _draw_classification_row(pdf: FPDF, classification: str, value1: str, value2: str = None, value3: str = None):
    ...
    if classification == "Clinical Safety, Patient Care, and Quality Assurance":
        classification = "Clin. Safety"
    elif classification == "Imaging Principles and Instrumentation":
        classification = "Imag. Princ."
    wrapped_lines = textwrap.wrap(classification, width=28)
    num_lines = max(1, len(wrapped_lines))
    total_height = num_lines * line_height
    x_start = pdf.get_x()
    y_start = pdf.get_y()
    for i, txt in enumerate(wrapped_lines):
        border_mode = "LTR" if i == 0 else ("LRB" if i == num_lines - 1 else "LR")
        pdf.cell(65, line_height, txt, border=border_mode, ln=1 if i < num_lines -1 else 0, align='L')
        if i < num_lines -1:
            pdf.set_x(x_start)
    pdf.set_xy(x_start + 65, y_start)
    pdf.cell(22, total_height, str(value1), border="LTRB", align='C')
    if value2 is not None:
        pdf.cell(22, total_height, str(value2), border="LTRB", align='C')
    if value3 is not None:
        pdf.cell(70, total_height, str(value3), border="LTRB", ln=1, align='C')
    else:
        pdf.ln()

def get_feedback(percent: float) -> str:
    """Devuelve el comentario de retroalimentación."""
    if percent >= 95:   return "Excellent"
    elif 80 <= percent <= 94:  return "Satisfactory"
    elif 61 <= percent <= 79:  return "May Require Further Study"
    elif percent <= 60: return "Requires Further Study"
    return "Unknown"

def generate_pdf(user_data, score, status, photo_path=None):
    """Genera el PDF."""
    pdf = CustomPDF()
    pdf.set_top_margin(30)  # Aumenta el margen superior (ajusta este valor)
    pdf.add_page()

    #  El logo ahora está en el encabezado (todas las páginas)
